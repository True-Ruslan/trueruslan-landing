...
test('AI index content maintenance is explicit owner-triggered and minimally privileged', () => {
  ...
  assert.match(source, /^\s{4}environment: ai-acceptance$/m);
  assert.doesNotMatch(source, /contents:\s*write|issues:\s*write|pull-requests:\s*write/);
});

test('AI index content maintenance binds the command to the current PR head before provider access', () => {
  const source = readWorkflow();
...
test('AI index content maintenance executes trusted indexing code and rejects unsafe candidate inputs', () => {
  ...
  assert.match(source, /find candidate\/docs .* -type l/);
  assert.match(source, /cp trusted\/scripts\/ai-index\.js candidate\/scripts\/ai-index\.js/);
  assert.match(source, /cp trusted\/scripts\/ai-corpus\.js candidate\/scripts\/ai-corpus\.js/);
  assert.match(source, /cp trusted\/scripts\/ai-config\.js candidate\/scripts\/ai-config\.js/);
...
test('AI index content maintenance refreshes only after preflight and preserves bounded evidence', () => {
  const source = readWorkflow();

  const preflight = source.indexOf('Reject executable or authority changes');
  const secretUse = source.indexOf('OPENROUTER_API_KEY: ${{ secrets.OPENROUTER_API_KEY }}');
  assert.ok(preflight >= 0 && secretUse > preflight, 'provider secret must appear only after candidate preflight');
  assert.match(source, /node scripts\/ai-index\.js/);
