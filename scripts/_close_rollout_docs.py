...
required = {
    STATE: ['### P2.4a — Canonical link rollout and first custom-host telemetry — DONE', 'Cloudflare provider telemetry for custom hostname — VERIFIED'],
    ROADMAP: ['### P2.4a Canonical link rollout and first custom-host telemetry — DONE', 'Vlezet flagship case study'],
    CHANGELOG: ['## P2.4a — Canonical link rollout and first custom-host telemetry', 'Provider dashboard telemetry for custom hostname — verified'],
}
...
if 'Cloudflare provider telemetry for custom hostname — OBSERVATION PENDING' in STATE.read_text(encoding='utf-8'):
    raise SystemExit('stale provider telemetry state remains')

print('Canonical link rollout documentation synchronized.')
