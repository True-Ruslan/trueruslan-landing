...
test('PROJECT_STATE records P3.6A readiness acceptance without claiming P3.6 measurement acceptance', () => {
  ...
  assert.match(
    state,
    /## 6\. Approved next product slice[\s\S]*Continue with:[\s\S]*\*\*C7 — production baseline \+ P3\.6 handoff — NEXT IMPLEMENTATION SLICE\.\*\*[\s\S]*P3\.6 remains \*\*NEXT \/ WAITING FOR EXTERNAL EVIDENCE\*\* in parallel/,
  );
