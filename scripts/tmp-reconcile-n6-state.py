...
def reconcile_project_state() -> None:
    ...
    anchor = (
        "Repository readiness, generated artifact, deployed production, search-engine observation "
        "и external-product acceptance остаются разными фактами.\n\n"
    )
    section = """## 0.6 2026-08-14 — N6 full-site editorial UX + bounded copy polish — PRODUCTION ACCEPTED
...
"""
    text = insert_after(text, anchor, section, "## 0.6 2026-08-14", "PROJECT_STATE section")
...
def reconcile_roadmap() -> None:
    ...
    anchor = (
        "> Current state — `docs/PROJECT_STATE.md`; history — `docs/CHANGELOG.md`; specification — "
        "`docs/keystone/specs/2026-08-05-portfolio-1-0-evidence-first.md`.\n\n"
    )
    section = """## 2026-08-14 accepted N6 full-site editorial UX + bounded copy polish
...
"""
    text = insert_after(text, anchor, section, "## 2026-08-14 accepted N6", "ROADMAP section")
    path.write_text(text)
...
def reconcile_changelog() -> None:
    ...
    anchor = (
        "> Current state — `docs/PROJECT_STATE.md`; next steps — `docs/ROADMAP.md`; specification — "
        "`docs/keystone/specs/2026-08-05-portfolio-1-0-evidence-first.md`.\n\n"
    )
    section = """## 2026-08-14 — N6 full-site editorial UX + bounded copy polish — PRODUCTION ACCEPTED
...
"""
    text = insert_after(text, anchor, section, "## 2026-08-14 — N6", "CHANGELOG section")
    path.write_text(text)


def verify() -> None:
    checks = {
        Path("docs/PROJECT_STATE.md"): "## 0.6 2026-08-14 — N6 full-site editorial UX + bounded copy polish — PRODUCTION ACCEPTED",
        Path("docs/ROADMAP.md"): "## 2026-08-14 accepted N6 full-site editorial UX + bounded copy polish",
        Path("docs/CHANGELOG.md"): "## 2026-08-14 — N6 full-site editorial UX + bounded copy polish — PRODUCTION ACCEPTED",
    }
    for path, marker in checks.items():
        text = path.read_text()
        assert text.count(marker) == 1, f"{path}: reconciliation marker count != 1"
        assert "controlled launch" in text
