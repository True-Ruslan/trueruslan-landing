...
test('freshness reconciliation records current truth without promoting lifecycle, installed acceptance or measurement boundaries', () => {
  ...
  assert.equal(vlezetEvidence.lastVerified, '2026-08-16');
  ...
  assert.ok(vlezetEvidence.versions.some(({label, value}) => label === 'Active product slice' && /M8\.3 Precision Reference Calibration active in Draft PR #92.*Testing Policy Phase A.*P0 IndexedDB persistence hardening.*TDD RED.*not product-owner accepted, merged or released/i.test(value)));
  ...
  const draft = signal(vlezetEvidence, 'https://github.com/True-Ruslan/vlezet/pull/92');
  ...
  assert.match(timelineEntry(vlezetHistory, 'vlezet', 'current').title, /M8\.3 Precision Reference Calibration.*active.*Draft.*RED/i);
  ...
  const installed031 = signal(livingworldEvidence, 'https://github.com/True-Ruslan/villAIgence/blob/1.21.1/docs/livingworld/VALIDATION_0.3.1_CORRECTIVE_INSTALLED.md');
  assert.equal(installed031.state, 'failed');
  assert.match(installed031.scope, /amber-pine-314.*did not recall/i);
  assert.equal(signal(livingworldEvidence, 'https://github.com/True-Ruslan/villAIgence/pull/169').state, 'merged');
  const release032 = signal(livingworldEvidence, 'https://github.com/True-Ruslan/villAIgence/releases/tag/0.3.2%2B1.21.1');
  ...
  const plan032 = signal(livingworldEvidence, 'https://github.com/True-Ruslan/villAIgence/pull/171');
  ...
  assert.equal(portfolioEvidence.lastVerified, '2026-08-16');
