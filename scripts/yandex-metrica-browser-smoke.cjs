...
function assertInitialState(current, intercepted, label) {
  if (intercepted.length !== 0) throw new Error(`${label}: expected zero Yandex requests, got ${intercepted.length}`);
  if (current.choice !== null || current.disabled !== true || !current.promptVisible || current.providerScripts.length !== 0) {
    throw new Error(`${label}: unexpected state ${JSON.stringify(current)}`);
  }
  if (current.reopenControlPresent) throw new Error(`${label}: reopen control must not exist ${JSON.stringify(current)}`);
  if (!current.promptText.includes('Cookies для статистики?') || current.grantText !== 'Разрешить' || current.denyText !== 'Не разрешать') {
    throw new Error(`${label}: compact RU copy is wrong ${JSON.stringify(current)}`);
  }
  if (JSON.stringify(current.grantVisual) !== JSON.stringify(current.denyVisual)) {
    throw new Error(`${label}: grant and refusal must have equivalent visual weight ${JSON.stringify(current)}`);
  }
  if (current.promptWidth > 520 || current.promptHeight > 64) {
    throw new Error(`${label}: prompt is not compact enough ${JSON.stringify({width: current.promptWidth, height: current.promptHeight})}`);
  }
}

async function assertDenialLifecycle(browser, baseUrl) {
  ...
  try {
    ...
    assertInitialState(current, intercepted, 'before consent');
    ...
    if (!current.promptVisible || current.choice !== null || current.reopenControlPresent || intercepted.length !== 0) {
      throw new Error(`prompt persists past seven seconds: unexpected state ${JSON.stringify(current)}`);
    }
    await assertNoMetricaCookies(context, 'after seven seconds without choice');
    ...
    if (current.choice !== 'denied' || current.disabled !== true || current.promptPresent || current.reopenControlPresent) {
      throw new Error(`after denial: one-shot UI must be gone with no reopen control ${JSON.stringify(current)}`);
    }
    await assertNoMetricaCookies(context, 'after denial');

    await page.reload({waitUntil: 'networkidle'});
    current = await state(page);
    if (intercepted.length !== 0 || current.choice !== 'denied' || current.disabled !== true || current.providerScripts.length !== 0 || current.promptPresent || current.reopenControlPresent) {
      throw new Error(`after denial reload: provider must remain disabled and UI absent ${JSON.stringify(current)}`);
    }
    diagnostics.assertClean('metrica-consent:denial-lifecycle');

    return {
      beforeConsentRequests: 0,
      afterSevenSecondsRequests: 0,
      afterDenialRequests: 0,
      promptPersistsWithoutChoice: true,
...
async function assertGrantLifecycle(browser, baseUrl) {
  const runtime = await createScenarioPage(browser, {
    viewport: VIEWPORTS.mobile,
    colorScheme: 'dark',
    reducedMotion: 'reduce',
  ...
  try {
    await installYandexInterception(context, intercepted);
    const response = await page.goto(`${baseUrl}/`, {waitUntil: 'networkidle'});
    if (!response?.ok()) throw new Error(`Metrica grant route HTTP ${response?.status() ?? 'none'}`);

    let current = await state(page);
    assertInitialState(current, intercepted, 'before grant');
    ...
    if (current.choice !== 'granted' || current.disabled !== false || current.providerScripts[0] !== PROVIDER_SRC || current.promptPresent || current.reopenControlPresent) {
      throw new Error(`after grant: one-shot UI must be gone with no reopen control ${JSON.stringify(current)}`);
    }
    ...
    const requestsBeforeReload = intercepted.length;
    await page.reload({waitUntil: 'networkidle'});
    current = await state(page);
    if (intercepted.length !== requestsBeforeReload + 1 || current.choice !== 'granted' || current.disabled !== false || current.providerScripts.length !== 1 || current.promptPresent || current.reopenControlPresent) {
      throw new Error(`after grant reload: persisted grant must load provider without recreating consent UI ${JSON.stringify(current)}`);
    }
    diagnostics.assertClean('metrica-consent:grant-lifecycle');

    return {
      grantRequests: 1,
      reloadGrantRequests: 1,
...
async function assertEnglishCopy(browser, baseUrl) {
  ...
  try {
    ...
    if (current.reopenControlPresent) {
      throw new Error(`English consent UI unexpectedly exposes a reopen control: ${JSON.stringify(current)}`);
    }
    if (JSON.stringify(current.grantVisual) !== JSON.stringify(current.denyVisual)) {
      throw new Error(`English grant and refusal must have equivalent visual weight ${JSON.stringify(current)}`);
    }
    return {localized: true, purposeSpecific: true, equivalentChoices: true, reopenControlAbsent: true};
  } finally {
    await runtime.close();
  }
}
...
async function main() {
  ...
  try {
    ...
    const denial = await assertDenialLifecycle(browser, serverRuntime.baseUrl);
    const grant = await assertGrantLifecycle(browser, serverRuntime.baseUrl);
